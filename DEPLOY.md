...
G-Player needs a **real container** (not serverless): it spawns **ffmpeg** for MKV
...
4. (Recommended) In **Environment**, set **`SITE_PASSWORD`** to lock the public URL
   behind HTTP Basic auth (`SITE_USER` defaults to `G-Player`). Without it, anyone with
...
- **Any VPS / home server** — `docker build -t G-Player . && docker run -p 3000:3000 -e SITE_PASSWORD=… G-Player`.
  Best for unmetered bandwidth and lowest latency to your IPTV provider.

## Run the container locally

```bash
docker build -t G-Player .
docker run --rm -p 3000:3000 -e SITE_PASSWORD=changeme G-Player
# open http://localhost:3000  (user: G-Player)
```
