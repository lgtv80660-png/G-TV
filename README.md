# G-Player — Cinematic IPTV / Xtream Web Player
...
> Bring your own IPTV subscription. G-Player is just the player.
...
Browsers can't call IPTV servers directly (CORS) and can't natively play MPEG-TS. G-Player runs a
...
- **Credentials** — httpOnly session cookie on this device + an optional saved-profile list in
  ...
  own IPTV provider.
- **Favourites / Continue-Watching / Recent** — `localStorage` (`G-Player-library`).
...
- **Movies/Series** are typically `.mp4` (native). **`.mkv` / `.avi`** often can't play in any
  browser — G-Player shows a clear notice; use VLC for those, or add the optional ffmpeg remux later.
